...
# ----------------------------
#
# These tests are designed to validate ANY Meetings API implementation.
# Your implementation must provide a fixture named `meetings_client`.
#
...
def meetings_spec() -> dict[str, Any]:
    if not SPEC_PATH.exists():
        raise AssertionError(
            f"Meetings OpenAPI spec not found at {SPEC_PATH}. "
            "Expected it under src/tests/schemas/workplace/Meetings.json"
        )
    return json.loads(SPEC_PATH.read_text(encoding="utf-8"))


def _get_request_callable(meetings_client: Any):
    if callable(meetings_client):
        return meetings_client
    req = getattr(meetings_client, "request", None)
    if callable(req):
        return req
    raise AssertionError(
        "The `meetings_client` fixture must return either a callable or an object "
...
@pytest.fixture
def meetings_cases(meetings_spec: dict[str, Any]) -> list[OperationCase]:
    cases = _build_operation_cases(meetings_spec)
    if not cases:
...
def test_meetings_spec_is_openapi_3(meetings_spec: dict[str, Any]) -> None:
    assert meetings_spec.get("openapi", "").startswith("3."), "Expected OpenAPI 3.x spec"
    assert meetings_spec.get("info", {}).get("title") == "Meetings"
    assert "paths" in meetings_spec and isinstance(meetings_spec["paths"], Mapping)


def test_meetings_operations_have_operation_ids(meetings_cases: list[OperationCase]) -> None:
    missing = [c for c in meetings_cases if not c.operation_id]
    assert not missing, "All operations should have an operationId (or derived id)."


def test_meetings_embedded_json_schemas_validate(meetings_cases: list[OperationCase]) -> None:
    for case in meetings_cases:
...
def test_meetings_placeholder(case: Any) -> None:
    assert case is None
...
def test_meetings_operation_contract(
    ...
    meetings_case: OperationCase,
    respx_mock: Any,
) -> None:
    request = _get_request_callable(meetings_client)

    formatted_path = _format_path(meetings_case.path, meetings_case.path_params)
    url = f"{BASE_URL}{formatted_path}"

    response_payload: Any = None
    if meetings_case.response_schema is not None:
        response_payload = _example_from_schema(meetings_spec, meetings_case.response_schema)
        if response_payload is None:
            response_payload = {}
        _validate(response_payload, meetings_case.response_schema)

    route_kwargs: dict[str, Any] = {"status_code": meetings_case.status_code}
    if meetings_case.response_schema is not None:
        route_kwargs["json"] = response_payload
    route = respx_mock.request(meetings_case.method, url).mock(
        return_value=httpx.Response(**route_kwargs)
    )

    result = request(
        meetings_case.method,
        meetings_case.path,
        path_params=meetings_case.path_params or None,
        params=meetings_case.query_params or None,
        json=meetings_case.request_json,
    )

    if isinstance(result, httpx.Response):
        assert result.status_code == meetings_case.status_code
        got = result.json() if result.content else None
    else:
        got = result

    assert route.called, f"Client did not call {meetings_case.method} {url}"

    call = route.calls[-1].request

    if meetings_case.query_params:
        for k, v in meetings_case.query_params.items():
            assert call.url.params.get(k) == str(v)

    if meetings_case.request_json is not None and meetings_case.method in {"POST", "PUT", "PATCH"}:
        ...
        assert sent == meetings_case.request_json

    if meetings_case.response_schema is not None:
        _validate(got, meetings_case.response_schema)
        assert got == response_payload
    else:
        assert got is None


def test_meetings_client_uses_httpx_transport(meetings_client: Any) -> None:
    req = _get_request_callable(meetings_client)
    assert callable(req)
